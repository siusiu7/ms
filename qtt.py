...
@description: 根据 Surge or Thor 请求内容生成相应的 js 请求脚本

无限生成qtt签到脚本
抓包找到https://api.1sapp.com/sign/info?
用py3种的该脚本生成对应的js脚本
暂时没有首页奖励和阅读奖励
"""
...
def extract_har(file):
  ...
  if params:
    data = [(i['name'], i.get('value', '')) for i in params]
...
const tokenVal = '&' + urlVal.match(/token=[a-zA-Z0-9_-]+/)[0]
const uuidVal = '&' + urlVal.match(/uuid=[a-zA-Z0-9_-]+/)[0]
const signVal = tokenVal + uuidVal
const signXTKVal = headerVal['X-Tk']
...
function bind() {
  return new Promise((resolve, reject) => {
    const url = {
      url: 'http://api.1sapp.com/lx/recallmember/bindTeacher?' + vsign + '&telephone=' + signinfo.login.data.telephone,
      headers: {
        'Host': 'api.1sapp.com',
        'X-Tk': signXTKVal,
      }
