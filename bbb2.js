...
function checkWaterNum() {
    return new Promise((resolve, reject) => {
        ...
        $.post(checkwaternum, async (error, response, data) => {
            $.log('\n🔔开始查询喝水杯数\n')
            const waternum = JSON.parse(removeByteOrderMark(data))
...
function adtype() {
    return new Promise((resolve, reject) => {
        let adtypeid ={
            url: `${BBB_API}user/adtype`,
            headers: JSON.parse(CookieVal[i]),
        }
        $.post(adtypeid,async(error, response, data) =>{
            const stradtype = JSON.parse(data)
            if(stradtype.ad_type == 1){
                $.log('\n🔔等待30秒后开始领取超过步数限额金币\n')
                await $.wait(30000)
                await no_callback()
            }
...
function no_callback() {
    return new Promise((resolve, reject) => {
        let callbackid ={
            url: `${BBB_API}you/no_callback`,
            headers: JSON.parse(CookieVal[i]),
            body: `you=0&type=13`,
        }
        $.post(callbackid,async(error, response, data) =>{
            const callback = JSON.parse(data)
            if(callback.code == 1){
                await doneJinS();
            }
    ...
    return new Promise((resolve, reject) => {
        ...
        let uploadtime = {
            ...
            headers: {
                ...
                "Host": "wapunionstatis.dfxwdc.com",
                "Referer": "https://toutiao.eastday.com/?qid=qid10267",
